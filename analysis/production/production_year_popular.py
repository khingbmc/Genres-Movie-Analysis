...
def analyse(production, year, mem):
    ...
    for idx, data in df_movie.iterrows():
        ...
        for i in json.loads(data.production_companies):
            if i['name'] == production and 0 <= int(check_year)-year <= 6:
...
def plotgraph(production, year, check):
    ...
    bar = pygal.Pie(fill=True, interpolate='cubic', style=NeonStyle)
    bar.title = production+' genres popularity static in '+str(year)+'-'+str(year+6)
    ...
    bar.render_to_file('..\..\web_project\static\svg\production\DC_Marvel\%s_%s.svg'%(production.replace(' ', '_'), year))
